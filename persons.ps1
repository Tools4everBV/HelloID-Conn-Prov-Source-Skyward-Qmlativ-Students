...
if($null -eq $configuration){
    ...
    #}
#    $config = [ordered]@{
#        BaseURI = "https://sandbox.skyward.com/Sandbox2019API"
#        ClientKey = "Tools4Ever"
#        ClientSecret = "TGOYGHEopulQkC5davxMFznF/nVcK/6GyrCR74Dh3nf9/9DX9OVgY/cKU6gCh6fdU+KxyxkvPxo/XOA+K8fdvg=="
#        PageSize = "2500"
#        EntityId = "1"
#        SchoolYearId = ""
#        FiscalYearId = ""
...
#region Enrichment Data - TODO: Move to CSV File.
$EnrichmentData = `
'Abbr,EntityID
' | ConvertFrom-Csv
#endregion
...
try{
    $mc = Measure-Command{
		...
		#### ENRICHMENT DATA #####
		# Import Enrichment data here.
		# 2021-11-23 T4e JA - PENDING TODO.  Need Site Abbreviations, OU Mapping, etc.
		# $EnrichmentData = Import-CSV ?????
		$EnrichmentData_ht_EntityID = $EnrichmentData | Group-ObjectHashtable -Property 'EntityID'
		$EnrichmentData_empty = @{}
			$EnrichmentData[0].PSObject.Properties.ForEach({$EnrichmentData_empty[$_.name -Replace '\W','_'] = ''})
    }
	...
	foreach($p in $Students)
	{
		...
		foreach($enrollment in $Enrollments_ht["$($p.StudentID)"])
		{
			...
			$contract = [ordered]@{
				...
				Team_Name			= ''
				Manager_DisplayName = ''
				EnrichmentData 		= $(if($null -ne $EnrichmentData_ht_EntityID["$($enrollment.EntityID)"]){$EnrichmentData_ht_EntityID["$($enrollment.EntityID)"][0]}else{$EnrichmentData_empty})
                #ADGroups            = ""
			}
		...
		foreach($ss in $StudentSection_ht["$($p.StudentID)"])
		{
			...
			$contract = [ordered]@{
				...
				EnrichmentData 		= $(if($null -ne $EnrichmentData_ht_EntityID["$($_Section.EntityID)"]){$EnrichmentData_ht_EntityID["$($_Section.EntityID)"][0]}else{$EnrichmentData_empty})
